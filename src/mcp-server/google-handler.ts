...
app.get('/auth/authorize', async (c) => {
  ...
  console.log('Authorize Endpoint - OAuth Request Info:', JSON.stringify(oauthReqInfo, null, 4))
  ...
  return renderApprovalDialog(c.req.raw, {
    ...
    server: {
      description: 'This MCP Server provides persistent memory storage for AI agents.',
  ...
  return new Response(null, {
    headers: {
      ...
      location: getUpstreamAuthorizeUrl({
        ...
        redirectUri: new URL('/auth/callback', c.req.raw.url).href,
        scope: 'email profile',
...
app.get('/auth/callback', async (c) => {
  ...
  const [accessToken, googleErrResponse] = await fetchUpstreamAuthToken({
    ...
    redirectUri: new URL('/auth/callback', c.req.url).href,
